...
const char * log_level_names[]= { "trace", "debug", "info", "warning", "error", "fatal" };

const char * log_level_name(int level) {
	if (level >= LOG_LEVEL_TRACE && level <= LOG_LEVEL_FATAL)
		return log_level_names[level - LOG_LEVEL_TRACE];
	return "unknown";
}

bool log_level_by_name(strseg_t name, int *lev) {
	int i;
	for (i= LOG_LEVEL_TRACE; i <= LOG_LEVEL_FATAL; i++)
		if (0 == strseg_cmp(name, STRSEG(log_level_names[i - LOG_LEVEL_TRACE]))) {
			if (lev) *lev= i;
			return true;
		}
	return false;
...
void log_set_filter(int value) {
	log_filter= (value > LOG_LEVEL_FATAL)? LOG_LEVEL_FATAL
		: (value < LOG_LEVEL_TRACE)? LOG_LEVEL_TRACE
		: value;
}
...
bool log_flush() {
	...
	while (log_buf_pos > 0) {
		...
		if (log_msg_lost) {
			n= snprintf(log_buffer + log_buf_pos, sizeof(log_buffer) - log_buf_pos, "warning: lost %d messages\n", log_msg_lost);

...
$dp->run('-v', '--exec-on-exit', '/bin/sh	-c	exit 42', '--version');
$dp->discard_response;
$dp->exit_is( 42, '--version exec()s' );

$dp= Test::DaemonProxy->new;
$dp->run('--exec-on-exit', '/bin/sh	-c	exit 42', '--help');
$dp->discard_response;
$dp->exit_is( 42, '--help exec()s' );

$dp= Test::DaemonProxy->new;
$dp->run('--exec-on-exit', '/bin/sh	-c	exit 42', '--nonsense-argument-that-fails');
$dp->discard_response;
$dp->exit_is( 42, 'invalid cmdline exec()s' );

$dp= Test::DaemonProxy->new;
$dp->run('--exec-on-exit', '/bin/sh	-c	exit 42', '-c', 'config-file-that-doesnt-exist');
$dp->discard_response;
$dp->exit_is( 42, 'open cfgfile failure exec()s' );

for my $sig ( qw( SIGABRT SIGSEGV ) ) {
	$dp= Test::DaemonProxy->new;
	$dp->run('--exec-on-exit', '/bin/sh	-c	exit 42');
	sleep 0.1;
	kill $sig => $dp->dp_pid;
